...
from utilities_polycistron import (
    mirna_polycistron_data,
    mirna_members_data,
    check_motif,
    mirna_loading,
    sirna_data,
...
(members, scaffold_mirna, left_flank, right_flank, sequence) = mirna_polycistron_data(
    mirna_polycistrons_file, cluster_name, flank_size=200
)

# Get matural mirna
scaffold, all_members, mirna_in_polycistrons = mirna_members_data(
    mirna_in_polycistron_file, mirna_members, scaffold_mirna, members
)

# Check neccesary parameters and save
loops_motifs_check, spacer_motifs_check = check_motif(
    scaffold_mirna, sequence, all_members
)

motif_results = pd.concat([loops_motifs_check, spacer_motifs_check])
motif_results.to_csv(
    os.path.join(output_folder, f"{cluster_name}_motif_check_results.csv"), index=None
)
...
pairs, all_alignments = mirna_sirna_pairs(
    mirna_in_polycistrons, scaffold, all_sequences, type=pair_mode, k=0.4
)
...
with open(
    os.path.join(output_folder, f"{file_name}_polycistron_sequence.fa"),
    "w",
    encoding="utf-8",
) as f:
    f.write(f"> {file_name}\n")
...
elements_list, oligos = find_elements_polycistron(
