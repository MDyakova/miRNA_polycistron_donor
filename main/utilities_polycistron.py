...
def mirna_members_data(mirna_in_polycistron_file, mirna_members, scaffold, members):
    ...
    mirna_in_polycistrons = mirna_in_polycistrons[
        mirna_in_polycistrons["mir_id"].apply(lambda p: p in members)
    ]
    ...
    mirna_in_polycistrons["in_list"] = mirna_in_polycistrons["mirbase_name"].apply(
        lambda p: 1 if p in mirna_members else 0
    )
    mirna_in_polycistrons["index"] = mirna_in_polycistrons.index
    scaffold_copy = scaffold

    mirna_not_in_polycistron = mirna_in_polycistrons[
        mirna_in_polycistrons["in_list"] == 0
    ]
    for ind in mirna_not_in_polycistron["index"]:
        hairpin_i = scaffold_copy.split("[")[ind + 1].split("]")[0]
        hairpin_i = (
            scaffold_copy.split(hairpin_i)[0][-1:]
            + hairpin_i
            + scaffold.split(hairpin_i)[1][:1]
        )
...
def check_motif(scaffold, sequence, all_members):
    ...
    loops = [
        seq.split("]")[0].split("}")[1].split("{")[0] for seq in scaffold.split("[")[1:]
    ]
    ...
    loops_motifs_check = pd.DataFrame(
        loops_motifs_check, columns=("Sequence", "with_motif")
    )
    ...
    for spacer in spacers:
        is_correct = np.max(
            [
                ((spacer[i] == "C") & (spacer[i + 3] == "C"))
                for i in range(len(spacer) - 3)
            ]
        )
        spacer_motifs_check.append([spacer, is_correct])

    spacer_motifs_check = pd.DataFrame(
        spacer_motifs_check, columns=("Sequence", "with_motif")
    )
    spacer_motifs_check["mirna"] = all_members
    spacer_motifs_check["type"] = "spacer motif"

    # mirna without spacer motifs
    spacer_motifs_check = spacer_motifs_check[
        spacer_motifs_check["with_motif"] == False
    ]
...
def mirna_loading(mirna_in_polycistrons):
    ...
    """
    mirna_in_polycistrons["evidence1_n"] = mirna_in_polycistrons["evidence1"].apply(
        lambda p: source_number(p)
    )
    mirna_in_polycistrons["evidence2_n"] = mirna_in_polycistrons["evidence2"].apply(
        lambda p: source_number(p)
    )

    mirna_in_polycistrons["mirna_strand"] = (
        mirna_in_polycistrons["evidence1_n"] - mirna_in_polycistrons["evidence2_n"]
    )
    mirna_in_polycistrons["mirna_strand"] = mirna_in_polycistrons["mirna_strand"].apply(
...
def sirna_data(sirna_file, refseq_sequence, comp_dict, output_folder):
    ...
    with open(
        os.path.join(output_folder, "sirna_with_incorrect_energy.txt"), encoding="utf-8"
    ) as file:
        sirna_with_incorrect_energy = file.read()
    ...
    if "name" not in sirnas.columns:
        sirnas["name"] = (
            "si" + sirnas["Name"] + "_" + sirnas["start_mirna"].astype("str")
        )

    sirnas = sirnas[
        sirnas["name"].apply(lambda p: p not in sirna_with_incorrect_energy)
    ]
...
def mirna_sirna_pairs(
    ...
    for i in range(np.minimum(len(mirna_in_polycistrons), len(all_sequences))):
        ...
        hairpin_seq = (
            scaffold.split("[")[1:][i].split("]")[0].replace("{", "").replace("}", "")
        )
                ...
                [
                    ...
                    np.sum(
                        [
                            (s1 == s2)
                            for s1, s2 in zip(
                                mirna_seq[: len(mirna_seq) - delta], sirna_seq
                            )
                        ]
                    ),
                ]
    ...
    all_alignments_g = all_alignments.groupby(by=["sirna_name"], as_index=False).max()[
        ["sirna_name", "score"]
    ]
    all_alignments = pd.merge(
        all_alignments, all_alignments_g, on=["sirna_name"], suffixes=("", "_max")
    )
    all_alignments["k"] = all_alignments["score"] / all_alignments["score_max"]
    all_alignments_f = all_alignments[all_alignments["k"] >= k]
    all_alignments_c = all_alignments_f.groupby(
        by=["mirna_name"], as_index=False
    ).count()[["sirna_name", "mirna_name"]]
    ...
    if type == "similar":
        ...
        for mirna_name in all_alignments_c["mirna_name"]:
            all_alignments_i = all_alignments[
                (all_alignments["mirna_name"] == mirna_name)
                & (all_alignments["sirna_name"].apply(lambda p: p not in sirna_pairs))
            ]
            all_alignments_i = all_alignments_i.sort_values(
        ...
        pairs = pd.DataFrame(
            data={"sirna_names": sirna_pairs, "mirna_names": mirna_pairs}
        )
    else:
        sirna_pairs = []
        mirna_pairs = []
        for mirna_name in all_alignments_c["mirna_name"]:
            all_alignments_i = all_alignments_f[
                (all_alignments_f["mirna_name"] == mirna_name)
...
def new_sequence_function(mirna_structure_list, sirna_seq, compl_dict, delta=1):
    ...
    for step in range(len(mirna_structure_list[1])):
        if (mirna_structure_list[1][step].islower()) | (
            mirna_structure_list[0][step].islower()
        ):
            new_sequence.append(
                [
                    mirna_structure_list[0][step],
        ...
        elif (mirna_structure_list[1][step].isupper()) | (
            mirna_structure_list[0][step].isupper()
        ):
...
def get_new_sequence(
    ...
    """
    if strand == 0:
        new_sequence, new_sequence_list = new_sequence_function(
            mirna_structure_list, sirna_seq, compl_dict, delta=delta
        )
    if strand == 1:
    ...
    with open(rna_fold_file, "w", encoding="utf-8") as file:
        file.write("> old seq \n")
        file.write(hairpin_seq.replace("T", "U") + "\n")
        file.write("> new seq \n")
        file.write(new_sequence_r.replace("T", "U") + "\n")

    # Launch RNAfold
    with open(rna_fold_file, "r", encoding="utf-8") as infile, open(
        rna_fold_out_file, "w", encoding="utf-8"
    ) as outfile:
        subprocess.run(
            ["RNAfold", "-p", "-d2"],
            stdin=infile,
    ...
    with open(rna_fold_out_file, encoding="utf-8") as file:
        rnafold_output = file.readlines()
    ...
    structure_compare = [
        int(old_structure[i] == new_structure[i]) for i in range(len(old_structure))
    ]
...
def get_mature_sequence_coords(sequence):
    ...
    """
    start_nt0_list = [
        i for i in range(len(list(sequence[0]))) if list(sequence[0])[i].isupper()
    ]
    start_nt1_list = [
        i for i in range(len(list(sequence[1]))) if list(sequence[1])[i].isupper()
    ]
    ...
    end_nt0_list = [
        i for i in range(len(list(sequence[3]))) if list(sequence[3])[i].isupper()
    ]
    end_nt1_list = [
        i for i in range(len(list(sequence[4]))) if list(sequence[4])[i].isupper()
    ]
...
def energy_sites(
    ...
    left_seq1 = "".join(
        [
            list(filter(lambda p: p != " ", [a, b]))[0]
            for a, b in zip(left_side[0], left_side[1])
        ]
    )
    left_seq2 = "".join(
        [
            list(filter(lambda p: p != " ", [a, b]))[0]
            for a, b in zip(left_side[3], left_side[4])
        ]
    )[::-1]
    ...
    right_seq1 = "".join(
        [
            list(filter(lambda p: p != " ", [a, b]))[0]
            for a, b in zip(right_side[0], right_side[1])
        ]
    )
    right_seq2 = "".join(
        [
            list(filter(lambda p: p != " ", [a, b]))[0]
            for a, b in zip(right_side[3], right_side[4])
        ]
    )[::-1]

    with open(rna_cofold_file, "w", encoding="utf-8") as file:
        file.write("> sample left \n")
        file.write(left_seq1 + "&" + left_seq2 + "\n")
        file.write("> sample right \n")
        file.write(right_seq1 + "&" + right_seq2 + "\n")

    with open(rna_cofold_file, "r", encoding="utf-8") as infile, open(
        rna_cofold_out_file, "w", encoding="utf-8"
    ) as outfile:
        subprocess.run(
            ["RNAcofold", "-p"],
            stdin=infile,
    ...
    with open(rna_cofold_out_file, encoding="utf-8") as file:
        rnacofoldresults = file.readlines()

    left_energy = float(
        rnacofoldresults[4].split("delta G binding=")[-1].split("\n")[0]
    )
    right_energy = float(
        rnacofoldresults[9].split("delta G binding=")[-1].split("\n")[0]
    )

    return left_energy, right_energy


def get_new_scaffold(
    scaffold,
    ...
    with open(
        os.path.join(output_folder, "sirna_with_incorrect_energy.txt"),
        encoding="utf-8"
    ) as file:
        sirna_incorrect_e = file.readlines()
    for mirna_name in pairs["mirna_names"]:
        mirna_data = mirna_in_polycistrons[
            mirna_in_polycistrons["mirbase_name"] == mirna_name
        ]
        mirna_structure = mirna_data["structure"].max()
        hairpin_seq = all_alignments[all_alignments["mirna_name"] == mirna_name][
            "hairpin_seq"
        ].max()
        strand = mirna_in_polycistrons[
            mirna_in_polycistrons["mirbase_name"] == mirna_name
        ]["mirna_strand"].max()

        sirna_name = pairs[pairs["mirna_names"] == mirna_name]["sirna_names"].max()
        sirna_seq = all_alignments[all_alignments["sirna_name"] == sirna_name][
            "sirna_seq"
        ].max()
        ...
        if (
            (np.max(structure_compare) >= 5)
            | (len(np.where(structure_compare > 0)[0]) > 3)
            | (delta_energy < -5)
        ):
            (
                new_sequence,
            ...
            if (
                (np.max(structure_compare) >= 5)
                | (len(np.where(structure_compare > 2)[0]) > 3)
                | (delta_energy < -5)
            ):
                if (sirna_name + " " + mirna_name) not in sirna_incorrect_e:
                    with open(
                        os.path.join(output_folder, "sirna_with_structure_error.txt"),
                        "a",
                        encoding="utf-8",
                    ) as file:
                        file.write(sirna_name + " " + mirna_name + "\n")
        ...
        left_energy, right_energy = energy_sites(
            mature_seq_list,
            strand,
            rna_cofold_file,
            rna_cofold_out_file,
        ...
        if right_energy - left_energy > 0:
            ...
            if sirna_name not in sirna_incorrect_e:
                with open(
                    ...
                    file.write(sirna_name + "\n")

        if len(scaffold_new.split(hairpin_seq)) == 2:
            scaffold_new = scaffold_new.replace(
                hairpin_seq, new_sequence.replace("-", "")
            )
...
def structure_prob(file_name):
    ...
    probabilities = pd.DataFrame(
        [
            i.split(" ")
            for i in dp_data.split("%start of base pair probability data\n")[1]
            .split("\nshowpage")[0]
            .split("\n")
        ],
    ...
    probabilities_1 = (
        probabilities[probabilities["type"] == "ubox"]
        .groupby(by=["pos_1"], as_index=False)
        .max()
    )
    ...
    probabilities_2 = (
        probabilities[probabilities["type"] == "ubox"]
        .groupby(by=["pos_2"], as_index=False)
        .max()
    )
    ...
    probabilities = (
        probabilities.groupby(by=["pos"], as_index=False).max().sort_values("pos")
    )

    sequence = "".join(
        dp_data.split("/sequence")[1].split(") }")[0].split("\\\n")[1:-1]
    )

    probabilities_all = pd.DataFrame(data={"pos": np.arange(1, len(sequence) + 1)})
    probabilities_all = pd.merge(
        probabilities_all, probabilities, on=["pos"], how="left"
    ).fillna(0)

    structure = "".join(
        probabilities_all["prob"].apply(lambda p: "|" if p > 0.85 else ".")
    )
...
def structure_coords(file_name, probabilities_all):
    ...
    coords = [
        [float(c[1:-1].split(" ")[0]), float(c[1:-1].split(" ")[1])] for c in coords
    ]
...
def structure_picture(
    probabilities_all, all_sequences, all_names, sequence, picture_name, output_folder
):
    """
    ...
    for step, (seq, name) in enumerate(zip(all_sequences, all_names)):
        ...
        prob_seq = probabilities_all[
            (probabilities_all["pos"] >= seq_start)
            & (probabilities_all["pos"] < seq_end)
        ]
...
def full_scaffold_structures(
    ...
    with open(rna_fold_file, "w", encoding="utf-8") as file:
        file.write("> old seq \n")
        file.write(scaffold_clean.replace("T", "U") + "\n")
        file.write("> new seq \n")
        file.write(scaffold_new.replace("T", "U") + "\n")
    ...
    _, probabilities_all, sequence = structure_prob(file_name)
...
def hairpin_plots(
    ...
    pos = 0
    for step, (seq_old, seq_new, mirna_name, sirna_name) in enumerate(
        zip(all_old_sequences, all_new_sequences, mirna_names, sirna_names)
    ):
        with open(rna_fold_file, "w", encoding="utf-8") as file:
            file.write("> old seq \n")
            file.write(seq_old.replace("T", "U") + "\n")
            file.write("> new seq \n")
            file.write(seq_new.replace("T", "U") + "\n")
        ...
        file_name = os.path.join(vienna_output_directory, "old_dp.ps")
        _, probabilities_all, _ = structure_prob(file_name)

        file_name = os.path.join(vienna_output_directory, "old_ss.ps")
        probabilities_all = structure_coords(file_name, probabilities_all)

        axs[pos].plot(
            probabilities_all["Y"], probabilities_all["X"], "k", linewidth=0.5
        )
        ...
        axs[pos].plot(
            probabilities_all["Y"], probabilities_all["X"], c=colors[step], linewidth=1
        )
...
def save_results(
    ...
    with open(
        os.path.join(output_folder, "pri-mirna sructures.txt"), "w", encoding="utf-8"
    ) as file:
        for struct_old, struct_new, mirna_name, sirna_name in zip(
            all_old_structure, all_new_structure, mirna_names, sirna_names
        ):
    ...
    with open(
        os.path.join(output_folder, "sirna_sequences.txt"), "w", encoding="utf-8"
    ) as file:
        for sirna_name, sirna_seq in all_sequences:
            if sirna_name in sirna_names:
                file.write("> " + sirna_name + "\n")
                file.write(sirna_seq + "\n")

    with open(
        os.path.join(output_folder, "sirna_hairpin_sequences.txt"),
        "w",
        encoding="utf-8",
    ) as file:
    ...
    with open(
        os.path.join(output_folder, "transcript_sequences.txt"), "w", encoding="utf-8"
    ) as file:
