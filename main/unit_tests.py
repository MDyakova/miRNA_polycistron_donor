...
def test_ensembl_data():
    ...
    assert (
        (start_gene == 241849684) & (end_gene==241859094)
...
def test_rnafold_mirna_results():
    ...
    assert (
    (mirnafold_data.loc[0]['delta_G_fold'] == -0.1)
...
def test_mirna_polycistron_data():
    ...
    members, _, _, _, _ = mirna_polycistron_data(file_name, cluster_name, flank_size = 200)

    assert (
    (members == ['MI0000081', 'MI0000085', 'MI0000079'])
...
def test_mirna_sirna_pairs():
    ...
    for ind in all_sequences_csv.index:
        all_sequences.append([all_sequences_csv.loc[ind, 'Name'],
                              all_sequences_csv.loc[ind, 'Sequence']])
    
    pairs, _ = mirna_sirna_pairs(mirna_in_polycistrons, scaffold, all_sequences, type = 'similar', k=0.4)
